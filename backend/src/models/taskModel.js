...
const getProjectTasks = (project_id, filters = {}, callback) => {
  ...
  const sort = allowedSort.includes(filters.sort) ? filters.sort : 'due_date';
...
const updateTask = (id, taskData, callback) => {
  ...
  pool.query(
    ...
    (error) => {
      ...
      getTaskById(id, callback); // 🔥 return full updated task
    }
  );
};
...
const updateTaskStatus = (id, status, callback) => {
  ...
  pool.query(updateSql, [status, id], (error) => {
    if (error) {
