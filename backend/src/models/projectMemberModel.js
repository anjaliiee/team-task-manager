...
const addProjectMember = (memberData, callback) => {
  ...
  pool.query(sql, [project_id, user_id, role], (error, results) => {
    ...
    callback(null, { id: results.insertId, project_id, user_id, role });
  });
};
...
const updateMemberRole = (project_id, user_id, role, callback) => {
  const sql = 'UPDATE project_members SET role = ? WHERE project_id = ? AND user_id = ?';
  pool.query(sql, [role, project_id, user_id], (error, results) => {
    if (error) {
